...
added_note = "Basic version of Basic Greedy Algorithm"
...
class GreedySearch:
    """
    Greedy Search algorithm for solving the Traveling Salesman Problem (TSP)
    using a 2D distance matrix.
    """

    def __init__(self, dist_matrix: List[List[float]]):
        """
        Initialize a new instance of the Greedy Search algorithm with the given distance matrix.
        ...
        self.start = random.randint(0, self.city_count - 1)

    def run(self) -> Tuple[List[int], float]:
        """
        Run the Greedy Search algorithm to find an approximate solution to the TSP.

        :return: A tuple containing the tour as a list of city indices and the total tour length.
        """
        tour = [self.start]
        tour_length = 0

        # Iterate until all cities are included in the tour
        while len(tour) < len(self.dist_matrix):
            min_dist = inf
            min_city = None

            # Find the closest city to the last city in the tour that has not been visited yet
            for city in range(len(self.dist_matrix)):
                if city in tour:
                    continue

                dist = self.dist_matrix[tour[-1]][city]
                if dist < min_dist:
                    min_dist = dist
                    min_city = city

            # Add the closest city to the tour and update the tour length
            tour.append(min_city)
            tour_length += min_dist

        # Add the distance from the last city in the tour back to the starting city
        tour_length += self.dist_matrix[tour[-1]][tour[0]]

        return tour, tour_length


greedy_search = GreedySearch(dist_matrix=dist_matrix)
tour, tour_length = greedy_search.run()
